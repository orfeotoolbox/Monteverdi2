...
// Fetch the no data flags if any
  ...
  if( settings==NULL )
    InitializeColorSetupSettings( buildContext->m_IsComplex );
...
void
VectorImageModel
::InitializeColorSetupSettings( bool isComplex )
{
  ...
  // Apply complex image mode.
  GetSettings().SetComplex( isComplex );

  // Store default grayscale-mode.
  if( isComplex )
    GetSettings().SetGrayChannel(
      m_Image->GetNumberOfComponentsPerPixel()
    );

  else if( m_Image->GetNumberOfComponentsPerPixel()<3 )
    {
